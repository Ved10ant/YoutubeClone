...
// Translation function using local API route to avoid CORS
...
const Comments = ({ videoId }: CommentsProps) => {
  ...
  return (
    <div className="space-y-6 mt-6 w-full px-2 sm:px-4 md:px-8 max-w-3xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
        <h2 className="text-base md:text-xl font-semibold">{comments.length} Comments</h2>
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-start w-full">
          ...
          <div className="flex-1 space-y-2 min-w-0 w-full">
            <Textarea
              ...
              className="min-h-20 resize-none border-0 border-b-2 rounded-none focus-visible:ring-0 text-xs sm:text-sm md:text-base w-full"
            />
      ...
      <div className="space-y-4 w-full">
        {comments.length === 0 ? (
          ...
          comments.map((comment) => (
            <div key={comment._id} className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-start w-full">
              <Avatar className="w-8 h-8 sm:w-10 sm:h-10">
              ...
              <div className="flex-1 min-w-0 w-full">
                ...
                <div className="mb-2">
                  {translatedTexts[comment._id] ? (
                    <div>
                      ...
                      <p className="text-xs sm:text-sm text-gray-600 italic mb-1">
                        {comment.commentbody}
                      </p>
                    ...
                    <p className="text-xs sm:text-sm">{comment.commentbody}</p>
                  )}
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-2">
                  <button
                    ...
                    className={`flex items-center gap-1 text-xs sm:text-sm ${isLiked(comment)
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 text-xs sm:text-sm ${isDisliked(comment)
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-xs sm:text-sm text-gray-600 hover:text-blue-600 disabled:opacity-50"
                  >
                  ...
                  {user && (String(comment.userid) === String(user._id || user.id)) && (
                    <button
                      ...
                      className="flex items-center gap-1 text-xs sm:text-sm text-red-600 hover:text-red-700 ml-auto"
                    >
                      <X className="w-4 h-4" />
                      <span>Delete</span>
                    </button>
